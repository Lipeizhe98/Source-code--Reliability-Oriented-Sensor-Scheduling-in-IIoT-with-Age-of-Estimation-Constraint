...
% --------------------------
T_Simulation = 10;
gamma_hist = cell(length(N0_total),1);
L_mat_hist = cell(length(N0_total),1);
time_hist = zeros(1,length(N0_total));
P_success_hist = cell(length(N0_total),1);
C_hist = cell(length(N0_total),1);
Upsilon = 10;

num_sensors = 10;
num_subproblem = 20;
time_sub_avg = zeros(length(N0_total),10);
...
p = 0.05 + transmit_power;
power = p;
Gain_Simulation = 60000/T_Simulation;
gamma = cell(length(N0_total),1); Tem = cell(length(N0_total),1);
C1 = cell(length(N0_total),1); P = cell(length(N0_total),1);
K = cell(length(N0_total),1); P_trace = cell(length(N0_total),1);

for i = 1:length(N0_total)
...
for i = 1:T_Simulation*Gain_Simulation
    % 对每个结果都做一遍这个循环
    for j = 1:length(N0_total)
       % 把对应的观测矩阵拿出来
        idx = find(gamma{j}(:,i) ~= 0);
        C1{j} = [];
        transmit_index = rand(length(idx),1);
        for k = 1:length(idx)
            if transmit_index(k)<=P_success_hist{j}(idx(k))
                C1{j} = [C1{j};C_hist{j}(idx(k),:)];
            end
        end
        % C1{j} = C(idx,:);
...
% 直接用平均温度去算可靠性增益
Tem_avg = cell(length(N0_total),1);
MTTF = cell(length(N0_total),1);

for j = 1:length(N0_total)
    Tem_avg{j} = mean(Tem{j},2);
end
...
for i = 1:num_sensors
    ...
    sensor_params(i).pi_t = 0.05 + transmit_power;
end
for j = 1:length(N0_total)
    for i = 1:num_sensors
        p = sensor_params(i);
...
MTTF_event = 10^2000*ones(length(N0_total),N_L);
for k = 1:length(N0_total)    
    % 直接利用L_mat去计算系统的MTTF
    for i = 1:N_L
        for j = 1:g_l
    ...
    % 还需要对比一下整体的能耗
    Energy(k) = sum(sum(gamma{k}))*transmit_power*delta_t/Gain_Simulation/T_Simulation*20000;
end
...
for i = 1:length(N0_total)
    ...
    plot([1:T_Simulation*Gain_Simulation]*0.1, P_trace{i}, '-.', 'Color', color_plot(i,:), ...
        'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', ['N_0=' num2str(N0_total(i)) 'dBm']);
end
% 坐标和标签
xlabel('Time');
ylabel('tr(P)');
...
% 自动生成标签
LabelStr = arrayfun(@(x) sprintf('$%d$', x), N0_total, 'UniformOutput', false);
% 创建和排序categorical
