...
% 直接用平均温度去算可靠性增益
Tem_avg = cell(length(Sensor_total),1);
MTTF = cell(length(Sensor_total),1);
for j = 1:length(Sensor_total)
...
for i = 1:50
    ...
    sensor_params(i).J = 1.5e6 + 10*randn();
...
for k = 1:length(Sensor_total)
    [N_L,g_l] = size(L_mat_hist{k});
    MTTF_event = 10^2000*ones(length(Sensor_total),N_L);
    % 直接利用L_mat去计算系统的MTTF
    for i = 1:N_L
        for j = 1:g_l
            MTTF_event(k,i) = min([MTTF_event(k,i),MTTF{k}(L_mat_hist{k}(i,j))]);
        end
    end
    MTTF_all(k) = max(MTTF_event(k,:));

    % 还需要对比一下整体的能耗
    Energy(k) = sum(sum(gamma{k}))*transmit_power*delta_t;
end
...
% 自动生成标签
LabelStr = arrayfun(@(x) sprintf('$%d$', x), Sensor_total, 'UniformOutput', false);
% 创建和排序categorical
...
% % 计算当前点的x坐标宽度
% gamma_all = [gamma1(3,:);gamma2(3,:);gamma3(3,:)];
% xWidth =1;
% Height = 0.2;
% Time_Int = 100;
% figure(9)
...
% axis([0 Time_Int,0.5,3+0.5])
% yticks([1 2 3 4])
% yticklabels({'RoSS','Min-AoI','CSC'})
% xlabel('Time step','Interpreter','latex','fontsize',16,'FontWeight','bold')
