...
% --------------------------
num_sensors = 20;
time_sub_avg = zeros(3,10);
time_opt_avg = zeros(1,10);
max_ite = 10;
time_sub_sum = [];
time_opt_sum = [];
for ite = 1:max_ite
    num_sensors_total = 11:3:14;
    for num_sensors = num_sensors_total
        W = 5 * eye(n);       % 过程噪声协方差
        R = 1 * eye(num_sensors);  % 观测噪声协方差
        ...
        p = 0.015 + transmit_power;

        X_hist = []; gamma_hist = [];        

        %% 对subproblem数量进行循环
        num_subproblem_total = 14:4:22;
        time_sub = zeros(length(num_subproblem_total),10);
        time_ite = 0;
        for num_subproblem = num_subproblem_total
            time_ite = time_ite + 1;
            ...
            clock2 = tic;
            Local_data = L_mat;
            [Local_A, Local_b] = local_constraint(L_mat, num_sensors, Upsilon, slot, P_success);
            [X_optimal, f_opt, time_opt(time_ite)] = slove_X(Local_data, p, zeta, T_init, aT, bT, cT, Tm, num_sensors, Upsilon, Local_A, Local_b);
            clock2_time(num_subproblem) = toc(clock2);
            ...
            parfor step = 1:num_subproblem
                ...
                X{step}= X_value; Obj_value{step} = f_value; time_sub1(1,step) = time;
            end
            clock1_time(num_subproblem) = toc(clock1);
            time_sub(time_ite,1:num_subproblem) = time_sub1; 
            time_sub1=[];
            % time_sub(time_ite,1) = clock1_time(num_subproblem);

            
            gamma = reshape(X{num_subproblem}(1:num_sensors*Upsilon,end), Upsilon, num_sensors)';
        end
        % save('20sensors_num_subproblem_comparision')

        time_sub_avg(:,num_sensors-9) = sum(time_sub, 2) ./ sum(time_sub ~= 0, 2);
        time_opt_avg(:,num_sensors-9) = mean(time_opt);
    end
    time_sub_sum = [time_sub_sum;time_sub_avg];
    time_opt_sum = [time_opt_sum;time_opt_avg];
end

save("Multisubproblems_v4.mat")
%% plot result
% x = num_subproblem_total;
x=num_sensors_total;
ind = num_sensors_total-min(num_sensors_total)+2;

time_sub1 = time_sub_sum(1:length(num_subproblem_total):size(time_sub_sum,1),:);
time_sub2 = time_sub_sum(2:length(num_subproblem_total):size(time_sub_sum,1),:);
time_sub3 = time_sub_sum(3:length(num_subproblem_total):size(time_sub_sum,1),:);



y_mean1 = mean(time_sub1,1)';
y_mean2 = mean(time_sub2,1)';
y_mean3 = mean(time_sub3,1)';

y_opt_mean = mean(time_opt_sum,1)';

% 计算上下边界
y_upper1 = max(time_sub1,[],1);
y_lower1 = min(time_sub1,[],1);
y_upper2 = max(time_sub2,[],1);
y_lower2 = min(time_sub2,[],1);
y_upper3 = max(time_sub3,[],1);
y_lower3 = min(time_sub3,[],1);

y_opt_upper = max(time_opt_sum,[],1);
y_opt_lower = min(time_opt_sum,[],1);

figure(8)
% 绘制阴影区域
h1 = fill([x, fliplr(x)], [y_upper1(ind), fliplr(y_lower1(ind))], ...
    [1.000, 0.894, 0.500], 'EdgeColor', 'none', 'FaceAlpha', 0.3); % 阴影带
set(h1, 'HandleVisibility', 'off');
hold on;
h2 = fill([x, fliplr(x)], [y_upper2(ind), fliplr(y_lower2(ind))], ...
    [0.988, 0.749, 0.600], 'EdgeColor', 'none', 'FaceAlpha', 0.3); % 阴影带
set(h2, 'HandleVisibility', 'off');
hold on;
h3 = fill([x, fliplr(x)], [y_upper3(ind), fliplr(y_lower3(ind))], ...
    [0.988, 0.698, 0.400], 'EdgeColor', 'none', 'FaceAlpha', 0.3); % 阴影带
set(h3, 'HandleVisibility', 'off');
hold on;
h4 = fill([x, fliplr(x)], [y_opt_upper(ind), fliplr(y_opt_lower(ind))], ...
    [0.600, 0.800, 0.600], 'EdgeColor', 'none', 'FaceAlpha', 0.3); % 阴影带
set(h4, 'HandleVisibility', 'off');
hold on;

% 绘制均值曲线
plot(x, y_mean1(ind), '-.s', 'Color', [0.929, 0.694, 0.125], ...
    'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', '14 sub-problems');
plot(x, y_mean2(ind), '-.<', 'Color', [0.850, 0.325, 0.098], ...
    'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', '18 sub-problems');
plot(x, y_mean3(ind), '-.^', 'Color', [0.929, 0.5, 0], ...
    'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', '22 sub-problems');

% 参考图 - 整体时间
plot(x, y_opt_mean(ind), '--o', 'Color', [0, 0.5, 0], ...
    'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Centralized algorithm');
% 添加标签
% 坐标和标签
...
%
% % 主图 - 子问题平均时间
% plot(x, time_sub_avg(1,:), '-.s', 'Color', [0.929, 0.694, 0.125], ...
%     'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Proposed algorithm with 2 sub-problems');
%
% plot(x, time_sub_avg(2,:), '-.<', 'Color', [0.850, 0.325, 0.098], ...
%     'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Proposed algorithm with 5 sub-problems');
%
% plot(x, time_sub_avg(3,:), '-.^', 'Color', [0.929, 0.5, 0], ...
%     'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Proposed algorithm with 8 sub-problems');
%
% % 参考图 - 整体时间
% plot(x, time_opt_avg, '--o', 'Color', [0, 0.5, 0], ...
%     'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Centralized algorithm');
%
% % 坐标和标签
