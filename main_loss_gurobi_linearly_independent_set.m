%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TII文章-面向可靠性的传感器调度
% 不同的传感器数量下，执行算法1的时间对比
% 和基于遍历的算法来对比，通过对比计算时间来看所提出方法的好处
%
% Peizhe Li
% 2026-04-10
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
P_success_hist = cell(length(Sensor_total),1);
C_hist = cell(length(Sensor_total),1);
Upsilon = 10;

t_alg1_hist = zeros(length(Sensor_total),T_Simulation);
t_alg2_hist = zeros(length(Sensor_total),T_Simulation);

for tims = 1:T_Simulation
    ite = 1;
    for num_sensors = Sensor_total
        num_subproblem = 30;
        ...
        %% 执行和算法1相关的迭代      
        [L_eigs, t_alg1_end] = run_algo1_per_eigenvalue(F, C, Gth);
        disp(['算法1运行时间',num2str(t_alg1_end)])

        t_alg2 = tic;
        observable_combinations = check_observability(F, C);
        t_alg2_end = toc(t_alg2);
        disp(['算法2运行时间',num2str(t_alg2_end)])

        t_alg1_hist(ite,tims) = t_alg1_end;
        t_alg2_hist(ite,tims) = t_alg2_end;
...
%% 画出相应的对比图
% t_alg1_hist 和 t_alg2_hist 维度为 [num_sensors, num_trials]，例如每个算法对每个传感器个数有多次蒙特卡洛仿真结果

% 计算每个传感器个数下的均值和上下界（标准差）
mean_alg1 = mean(t_alg1_hist, 2); % 计算每个传感器个数的均值
std_alg1 = std(t_alg1_hist, 0, 2); % 计算标准差
mean_alg2 = mean(t_alg2_hist, 2);
std_alg2 = std(t_alg2_hist, 0, 2);
...
% 定义颜色
color_fill = [1.000, 0.894, 0.500];  % 黄色
color_plot = [0.929, 0.694, 0.125];  % 黄色

% 绘制算法1的阴影区域和均值曲线
h1 = fill([Sensor_total, fliplr(Sensor_total)], ...
    [mean_alg1 + std_alg1; flipud(mean_alg1 - std_alg1)], ...
    color_fill, 'EdgeColor', 'none', 'FaceAlpha', 0.3); % 阴影带
set(h1, 'HandleVisibility', 'off');
plot(Sensor_total, mean_alg1, '-.s', 'Color', color_plot, ...
    'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Hierarchical Algorithm');

% 绘制算法2的阴影区域和均值曲线
color_fill = [0.988, 0.749, 0.600];  % 浅橙
color_plot = [0.850, 0.325, 0.098];  % 橙红
h2 = fill([Sensor_total, fliplr(Sensor_total)], ...
    [mean_alg2 + std_alg2; flipud(mean_alg2 - std_alg2)], ...
    color_fill, 'EdgeColor', 'none', 'FaceAlpha', 0.3); % 阴影带
set(h2, 'HandleVisibility', 'off');
plot(Sensor_total, mean_alg2, '-.s', 'Color', color_plot, ...
    'LineWidth', 1.8, 'MarkerSize', 8, 'DisplayName', 'Traversal Algorithm');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 利用遍历来找到所有可能的传感器组合
function observable_combinations = check_observability(F, C)
    % F: 状态转移矩阵 (n x n)
    % C: 观测矩阵 (m x n)，其中 m 为传感器的数量，n 为状态的维度

    [m, n] = size(C); % m为传感器数量，n为系统状态维度
    num_sensors = m; % 传感器的总数量
    state_dim = n; % 状态的维度
    
    observable_combinations = {}; % 初始化一个空的cell数组来存储可观的传感器组合
    
    % 初始化一个空的cell数组，用于存储传感器组合
    sensor_combinations = {};
    % 遍历所有可能的组合
    for k = 5
        % 使用nchoosek生成从n个元素中选择k个的组合
        combinations = nchoosek(1:m, k);  % 返回n个传感器中选择k个的所有组合
        % 将每次组合添加到结果中
        for i = 1:size(combinations, 1)
            sensor_combinations{end+1} = combinations(i, :);  % 将每个组合添加到cell数组中
        end
    end

    % for k = 1:2^num_sensors - 1
    for k = 1:length(sensor_combinations)
        % 获取当前组合的传感器
        selected_sensors = sensor_combinations{k}; % 获取当前组合的传感器索引
        % selected_sensors = find(bitget(k, 1:num_sensors)); % 获取当前组合的传感器索引
        C_selected = C(selected_sensors, :); % 选择对应的传感器矩阵
        
        % 计算可观性矩阵
        O = C_selected; % 初始可观性矩阵
        for i = 1:state_dim-1
            O = [O; C_selected * F^i]; % 累加每个阶数的观测矩阵
        end
        
        % 判断可观性：可观性矩阵的秩是否等于状态的维度
        if rank(O) == state_dim
            observable_combinations{end+1} = selected_sensors; % 将可观的传感器组合存入数组
        end
    end
end
...
% 循环运行算法1，对每个特征值
function [L_all_eigs, t_alg1_end] = run_algo1_per_eigenvalue(F, C, Gth)
% 计算所有特征值的几何重数
